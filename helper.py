...
def data_over_time(df,col):
    nations_over_time1=df.drop_duplicates(['Year',col])['Year'].value_counts().reset_index().sort_values('Year')
    nations_over_time1.rename(columns={'Year': 'Edition', 'count': col}, inplace=True)
    return nations_over_time1
...
def most_successful(df, sport):
    ...
    if sport != 'Overall':
        temp_df = temp_df[temp_df['Sport'] == sport]  # only that rows that have choose sport
    x = temp_df['Name'].value_counts().reset_index().head(15).merge(df, left_on='Name',
                                                                   right_on='Name', how='left')[
        ['Name', 'count', 'Sport', 'region']].drop_duplicates('Name')  # counts that particular athlete
...
def most_successful_countrywise(df, country):
    ...
    x = temp_df['Name'].value_counts().reset_index().head(10).merge(df, left_on='Name',
        right_on='Name', how='left')[['Name', 'count', 'Sport']].drop_duplicates('Name')
    x.rename(columns={'count': 'Medals'}, inplace=True)
    return x
...
def weight_vs_height(df,sport):
    ...
    if sport!='Overall':
        temp_df1 = athlete_df[athlete_df['Sport'] == sport]  # for a particular sport
        return temp_df1
    else:
        return athlete_df
